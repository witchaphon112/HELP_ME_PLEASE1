...
export default function InProgressIncidents() {
  ...
  useEffect(() => {
    const load = () => {
      ...
      setIncidents(all.filter((inc: IncidentMock) => inc.status === 'กำลังดำเนินการ'));
    };
    load();
  ...
  const updateStatus = (id: number, newStatus: IncidentMock['status']) => {
    ...
    setIncidents(updated.filter((inc: IncidentMock) => inc.status === 'กำลังดำเนินการ'));
  };

  return (
    <div className="staff-dashboard-container">
      <h2>เหตุการณ์กำลังดำเนินการ</h2>
